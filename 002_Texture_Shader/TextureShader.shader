...
Shader "Lein's Shader/TextureShader"      // 유니티의 Shader 메뉴에서 표시될 이름을 지정합니다.
{
    ...
    Properties
    {
        ...
        _MainTex ("Texture", 2D) = "white" {} // 텍스쳐가 없을 경우 하얀색으로 초기화합니다.
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // material에 적용할 텍스처를 선언합니다.
            sampler2D _MainTex;
            float4 _MainTex_ST;

            // vertex shader로 읽을 mesh data들을 정의합니다.
            // texture는 uv라는 인자를 받습니다. 위치를 표현할 때 x, y를 사용하는걸 생각하시면 됩니다.
            struct appdata {
            ...
            v2f vert(appdata v)
            {
                ...
                // texture를 UV 좌표계로 변환해서 v2f 타입의 o에 대입해줍니다.
            ...
            {
                // texture에 color까지 적용해야 하므로 가공이 필요합니다.
                // 우선 _MainTex의 각각의 UV좌표에 대응되는 컬러를 가져옵시다.
                fixed4 col = tex2D(_MainTex, i.uv);

                // texture의 색상에 _Color를 곱해봅시다. 
                col *= _Color;

                // 가공된 값, col을 반환하고 fragment shader 함수를 종료합니다.
                return col;
            }
